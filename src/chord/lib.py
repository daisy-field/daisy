...
def check_if_peer_is_predecessor(pred_id: int, peer_id: int, new_pred_id: int) -> bool:
    ...
    return ((peer_id < pred_id) and (new_pred_id not in range(peer_id + 1, pred_id + 1))) or (
            new_pred_id in range(pred_id + 1, peer_id + 1))
...
def check_if_peer_is_successor(succ_id: int, peer_id: int, new_succ_id: int) -> bool:
    ...
    if succ_id is None and new_succ_id != peer_id:  # if no pred set
        return True
    return ((peer_id > succ_id) and (new_succ_id not in range(succ_id + 1, peer_id + 1))) or (
            new_succ_id in range(peer_id + 1, succ_id + 1))


def check_if_peer_is_between(peer_id: int, intermediate_id: int, boundary_id) -> bool:
    return (peer_id > boundary_id and intermediate_id in range(boundary_id - 1, peer_id)) or (
            intermediate_id not in range(peer_id, boundary_id - 1))


def fingertable_to_string(fingertable: dict[int, tuple[int, tuple[str, int], StreamEndpoint]]) -> list[str]:
    return [('(' + f'{key}: {fingertable[key][0]}' + ')') for key in fingertable.keys()]


def close_and_remove_endpoint(ep_server: EndpointServer, ep_addr: tuple[str, int], ep: StreamEndpoint):
    """
    :param ep_server: 
...
def get_readable_endpoints(finger_values, succ_ep: StreamEndpoint, pred_ep: StreamEndpoint,
                           ep_server: EndpointServer) -> set[StreamEndpoint]:
    ...
    """
    readable_endpoints = {finger_ep for _, _, finger_ep in finger_values if finger_ep.poll()[0][1]}
