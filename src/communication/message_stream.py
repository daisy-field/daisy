...
# TODO bundling of socket functions
# TODO filter logging
# TODO filter exceptions
# TODO fix misc FIXME
# TODO docstrings, typehints
# TODO ASYNC CLEANUP (STARTED FLAG) -> NON BLOCKING
...
class StreamEndpoint:
    ...
    class EndpointSocket:
            ...
            while self._started:
                try:
                    ...
                    if self._remote_addr is None:
                        ...
                        self._remote_sock.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, self._send_b_size)
        ...
        def send(self, p_data: bytes) -> None:
    ...
    def start(self):
        ...
        if self._endpoint_type.value == 0:
            if self._multithreading:
                self._thread = threading.Thread(target=self._create_source)
                self._thread.start()
            else:
                self._create_source()
        else:
            if self._multithreading:
                self._thread = threading.Thread(target=self._create_sink)
                self._thread.start()
            else:
                self._create_sink()
    ...
    def stop(self):  # FIXME thread safety!
