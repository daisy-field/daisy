...
from daisy.chord.lib import (
    cleanup_dead_messages,
    fingertable_to_string,
    check_if_peer_is_between,
    check_if_peer_is_predecessor,
    check_if_peer_is_successor,
    get_readable_endpoints,
)
...
class Chordpeer:
    ...
    _max_fingers: int  # let m/max_fingers be the number of bits in the key/peer
    # identifiers. (copied from paper)

    _sent_messages: dict[uuid4, tuple[MessageOrigin, time, typing.Optional[int]]]
    ...
    def __init__(
        ...
        :param max_fingers: Max size of fingertable, also used for calculating peer
        ids on chord
        """
    ...
    def _send_message(
        self,
        ep: StreamEndpoint | None,
        remote_addr: tuple[str, int],
        message: Chordmessage,
    ):
        """Sends a Chordmessage over a given endpoint or temporarily creates a new
        one.

        # TODO Initiates clean up if the given endpoint is no longer usable,
                but sends the message nevertheless.

        Note: The given Endpoint may be None, or inactive. The peer to whom the
        Message should be sent may have failed, or left the chordring. Due to the
        asychronicity of the Endpoints sending message and immediately closing the
        endpoint may lead to losing the message.
        ...
        try:
            ep.send(message)
            self._logger.info(
                f"Sent Message of Type {message.message_type} "
                f"From {self._name, self._id} "
                f"To {remote_addr} "
                f"With {message.peer_tuple}"
            )
        except (
            AttributeError,
            RuntimeError,
        ) as e:  # attributeError if ep is none, runtimeError if ep is not started
            self._logger.warning(
                f"{e.__class__.__name__} ({e}) :: in _send_message: creating new ep "
                f"to send."
            )
    ...
    def _join(self, remote_addr: tuple[str, int]):
        ...
        # TODO retry after ttl of message runs out ->
            # check ttl of messages periodically?
        ...
        join_chord_message = Chordmessage(
    ...
    def _notify(self, remote_id: int, remote_addr: tuple[str, int]):
        """Creates and sends a notify message to validate a peer's predecessor. Used
        to repair successor pointers on the chord ring."""
        if self._predecessor[0] == self._id:
            self._logger.warning("Notify to self")
            return
    ...
    def _stabilize(self, remote_id: int, remote_addr: tuple[str, int]):
        """Creates and sends a stabilize message to the sucessor of a peer,
        initiating reparation of its successor and respective predecessor pointers.
        ...
        if (remote_id is None) or (remote_addr is None):
            self._logger.warning(
                f"Stabilize missing value (remote_id:{remote_id}, "
    ...
    def _update_finger(
        self, finger_tuple: tuple[int, tuple[str, int]], finger_index: int
    ):
        """Updates an entry in a peers fingertable by maintaining its endpoint and
        assigning current finger-values or removing the finger entirely.
    ...
    def _get_existing_or_new_endpoint(
        self, remote_id: int, remote_addr: tuple[str, int]
    ) -> Optional[StreamEndpoint]:
        """Checks whether an endpoint to a given peer exists and returns it. Returns
        fresh endpoint if none exists.

        Note: Endpoints, or peers, may fail at any point within or outside the
        function. Any endpoint returned by this function may be dead. This problem
        remains to be solved somehow.
    ...
    def _fix_fingers(self):
        """Iterates the finger indices and initiates lookups for the respective
        peers; defaults to the successor.

        Note: this method does not actually update any finger table entries. It will
        return early if a peer has no successor.
        """
        # if self._successor is None:
        self._logger.info("Starting to update Fingers... ")
        # return

        for i in range(self._max_fingers):
            finger = self._id + 2**i % (2**self._max_fingers)
            if check_if_peer_is_successor(
                self._successor[0], self._id, finger
            ):  # fixme why this?
                ...
                self._lookup_successor(finger, self._addr, message_id)

    def _get_closest_known_pred(
        self, peer_id: int
    ) -> tuple[int, tuple[str, int], StreamEndpoint] | None:
        """Finds the closest predecessor one peer knows for another peer in the
        fingertable. Defaults to None.
        ...
        if len(self._fingertable) == 0:
            self._logger.info(
                "In _get_closest_known_pred: empty fingertable; returning None"
            )
            return None
        ...
        for i in range(0, self._max_fingers, -1):
            try:
                ...
                if check_if_peer_is_between(self._id, finger[0], peer_id):
                    self._logger.info(
                        f"In _get_closest_known_pred: returning finger {finger[0]}"
                    )
                    return finger
        ...
        self._logger.info("In _get_closest_known_pred: returning None")

    def _lookup_successor(
        self, peer_id: int, peer_addr: tuple[str, int], message_id: uuid4
    ):
        """Checks whether a peer knows the sucessor of the given peer. If it knows
        the successor it will send back its (id, addr) pair, otherwise the next peer
        will be asked.

        :param peer_addr: chord address of peer who initially sent the find_successor
        request :param peer_id: chord id of peer whose successor should be found
        :message_id:
        """
        self._logger.info(f"looking up {peer_id}.")

        # second condition: if two peers are in ring A should not send B to B as its
        # successor
        if peer_addr == self._predecessor[1] or check_if_peer_is_predecessor(
            self._predecessor[0], self._id, peer_id
        ):
            self._logger.info(f"In _find_succ: found succ for {peer_id} as self ")
            succ_found_msg = Chordmessage(
                message_id=message_id,
                message_type=MessageType.LOOKUP_SUCC_RES,
                peer_tuple=(self._id, self._addr),
            )
            ...
            self._logger.info(
                f"In _find_succ: found succ for {peer_id} as "
                f"self._successor:{self._successor[0]} "
            )
            succ_found_msg = Chordmessage(
                message_id=message_id,
                message_type=MessageType.LOOKUP_SUCC_RES,
            ...
            ep = self._get_existing_or_new_endpoint(peer_id, peer_addr)
            self._send_message(
                ep, peer_addr, succ_found_msg
            )  # not closing ep if it comes from check
            ...
            try:
                ...
                self._logger.error(
                    ...
                    f"lookup; no ep for closest pred"
                    f"found in ft"
                )
                succ_found_msg = Chordmessage(
                    message_id=message_id,
    ...
    def _process_join(self, message: Chordmessage):
        ...
        if self._successor is None:
            self._logger.info(
                f"In _process_join: updating successor from None to {self._successor}."
            )
            self._successor = message.peer_tuple
            self._successor_endpoint = StreamEndpoint(
                name=f"succ-ep-{self._name}",
                remote_addr=message.peer_tuple[1],
                acceptor=False,
                multithreading=True,
                buffer_size=10000,
            )
        ...
        self._logger.info(
            f"In _process_join: initiating _find_succ for {message.peer_tuple}."
        )
    ...
    def _process_notify(self, message: Chordmessage):
        ...
        if message.peer_tuple[0] == self._id:
            return
        if check_if_peer_is_successor(
            self._successor[0], self._id, message.peer_tuple[0]
        ):
            self._logger.info(
                f"In _process_notify: updating successor from {self._successor} to "
                f"{message.peer_tuple}."
            )
            self._successor = message.peer_tuple
    ...
    def _process_stabilize(self, message: Chordmessage):
        """Handles incoming stabilize messages. Updates the predecessor of a peer or
        returns.

        :param message: Contains the id of a peer who believes the receiving peer is
        its successor.
        """
        ...
        elif check_if_peer_is_predecessor(
            self._predecessor[0], self._id, message.peer_tuple[0]
        ) and (self._id != message.peer_tuple[0]):
            self._predecessor = message.peer_tuple
            ep = self._get_existing_or_new_endpoint(*message.peer_tuple)
            if ep is None:
    ...
    def _process_lookup_succ_req(self, message: Chordmessage):
        """Handles incoming find successor request messages. Terminates the search if
        the request contains self, or initiates new find successor step.
        ...
        if message.peer_tuple[0] == self._id:
            self._logger.info(
                "In _process_find_succ_req: terminating search because find_succ on "
                "self."
            )
            return
        self._logger.info(
            f"Init _find_succ for {message.peer_tuple[0]}, In _process_find_succ_req."
        )
        self._lookup_successor(*message.peer_tuple, message.message_id)

    def _process_lookup_succ_res(self, message: Chordmessage):
        """Handles incoming find successor response messages. Updates the successor
        of a peer and initiates stabilize, updates a finger in the fingertable,
        depending on the message origin.
        ...
        if message.message_id not in self._sent_messages:
            self._logger.info(
                f"Received dead Message {message.peer_tuple}, "
                f"In _process_find_succ_res."
            )
            return

        msg_origin, _, msg_finger = self._sent_messages.pop(
            message.message_id, (None, None, None)
        )
        self._logger.info(
            f"Received {message.peer_tuple}, {msg_origin}, In _process_find_succ_res."
        )
        ...
        if msg_origin == MessageOrigin.FIX_FINGERS:
            self._logger.info(
                f"Initiating _update_finger for {msg_finger} "
                f"In _process_find_succ_res."
            )
            self._update_finger(message.peer_tuple, msg_finger)

    def run(self, bootstrap_addr: tuple[str, int] = None):
        """Starts the chord peer, joining an existing chord ring or bootstrapping a
        new one, before starting the periodic loop to maintain the ring and process
        incoming messages.

        :param bootstrap_addr: Remote address of peer to join existing ring, None to
        start new ring.
        """
        ...
        while True:
            ...
            if curr_time - last_refresh_time >= self._refresh_interval:
                try:
                    ...
                    self._logger.error(
                        f"{e.__class__.__name__} ({e}) :: could not Stabilize"
                    )
            ...
            for message in received_messages:
                ...
                self._logger.info(
                    f"In run: Received {msg_type.name} with {message.peer_tuple}"
                )
                match msg_type:
                    ...
                    case MessageType.NOTIFY:
                        ...
                        self._logger.info("NOTIFLY Processed: " + self.__str__())
    ...
    def _receive_on_endpoints(self) -> list[Chordmessage]:
        ...
        """
        endpoints = get_readable_endpoints(
            self._fingertable.values(),
            self._successor_endpoint,
            self._predecessor_endpoint,
        ...
        for endpoint in endpoints:
            try:
                while True:
                    messages.append(
                        typing.cast(Chordmessage, endpoint.receive(timeout=0))
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--remotePort", type=int, default=None, help="Port of remote peer"
    )
