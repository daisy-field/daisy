...
class Aggregator(ABC):
    ...
    def aggregate(self, *args, **kwargs) -> object:
        ...
        :param kwargs: Keyword arguments. Depends on the type of aggregator to be implemented.
...
class ModelAggregator(Aggregator):
    """Abstract model aggregator class, that uses the parameters of the given models and some internal states (of those
    parameters) to compute the aggregated state of the model. Must always be implemented if a new model aggregation
    strategy is used in the federated system.
    """

    @abstractmethod
    def aggregate(self, models_parameters: list[list[np.ndarray]]) -> list[np.ndarray]:
        """Calculates the aggregated values from the underlying object attributes and given model parameters and
        returns them.
...
class FedAvgAggregator(ModelAggregator):
    ...
    using only their weights (no internal state is kept); simply computing their average and using that as a result.

    Note this could also be done using gradients or losses or any other form of parameters from other types of models,
    however this was implemented with the federated_model.TFFederatedModel and its parameters in mind.
    """
...
class CumAggregator(ModelAggregator):
    """Cumulative Averaging (FedCum) is the base version of online averaging for models into a "global" version of the
    model. The cumulative average is computed by first computing the FedAvg over the batch of models provided to the
    aggregator in a single step, before using that average to update the average (i.e., all models in a batch are
    treated equally).
    ...
    def aggregate(self, models_parameters: list[list[np.ndarray]]) -> list[np.ndarray]:
        """Calculates the cumulative average for the given models' parameters and the previous cumulative average and
        returns it.
...
class SMAggregator(ModelAggregator):
    ...
    def aggregate(self, models_parameters: list[list[np.ndarray]]) -> list[np.ndarray]:
        """Calculates the simple moving average for the given models' parameters by averaging them first and then adding
        them to the window. Afterwards the delta between the new added values and the first (to be removed) value from
        the window is computed to aggregate the new moving average.
