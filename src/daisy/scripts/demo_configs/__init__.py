...
Pre-configured set of components of the federated IDS to perform the most simple
demonstration of the entire system using a simple client-server topology, consisting
of two federated IDS detection clients along the model aggregation server and two
additional aggregation servers, one for the prediction results and a second one for
the evaluation results.

Each module of this package contains one of the following components, that can either
be launched directly via the script (each containing a main) or via the command line,
the name scheme of which consists of "demo_202312_NAME" with the following options:

    * client - Online IDS client processing one of the two splits of the
    demonstration dataset (see below).
    * server - TODO
