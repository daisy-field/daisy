...
from daisy.data_sources import DataProcessor, SimpleDataSource


@pytest.fixture
def data_processor(example_dict):
    test_dict = example_dict.copy()
    data_source = SimpleDataSource(iter([test_dict]))
...
def test_no_added_func(data_processor, example_dict):
    ...
    assert next(data_processor) == eval_dict


def test_func_order(data_processor):
    ...
    result = typing.cast(dict, next(data_processor))

    assert all(map(lambda x: x.endswith("12345"), result.values()))


def test_single_func(data_processor):
...
def test_flatten_dict(flat_dict):
    data_source = SimpleDataSource(iter([flat_dict[0].copy()]))
    data_processor = DataProcessor(data_source=data_source)
    data_processor.flatten_dict()
    assert next(data_processor) == flat_dict[1]
...
def test_flatten_dict_value_error(test_dict):
    data_source = SimpleDataSource(iter([test_dict]))
    data_processor = DataProcessor(data_source=data_source)
    data_processor.flatten_dict()
